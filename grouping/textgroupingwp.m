function [textBox,comNumLine] = textgroupingwp(binaryImg,rgbImg)
    ...
    if length(cIndex)>=3
      ...
      [dist,distIndex] = sort(dist,2);
       for i = 1:length(cIndex)-1         
        if flag(cIndex(i)) ==0
            minIndex = findmindistwp(i,cIndex,dist(i,:),distIndex(i,:),...
            skelLength(cIndex),area(cIndex),flag(cIndex),orientation,...
            -1,-1,-1);
            if ~isempty(minIndex)
                    lineNum = lineNum+1;
                    ...
                    textLength(lineNum) = abs(bBox(minIndex,1)-bBox(cIndex(i),2))+1;
                    comNumLine(lineNum) = 2;
                    comDistLine(lineNum,1) = abs(bBox(minIndex,1)-bBox(cIndex(i),2))+1;
            end
        else
            minIndex = findmindistwp(i,cIndex,dist(i,:),distIndex(i,:),skelLength(cIndex),...
            area(cIndex),flag(cIndex),orientation,textLength(flag(cIndex(i))),bBox(cIndex,:),...
            slope(flag(cIndex(i))));
            if ~isempty(minIndex)
                ...
                comDistLine(flag(cIndex(i)),comNumLine(flag(cIndex(i)))) = abs(bBox(minIndex,1)-bBox(cIndex(i),2));
                textLength(flag(cIndex(i))) = mean(comDistLine(flag(cIndex(i)),1:comNumLine(flag(cIndex(i)))));
